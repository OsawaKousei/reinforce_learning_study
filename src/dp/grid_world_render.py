...
class Renderer:
    ...
    def render_v(self, v=None, policy=None, print_value=True):
        ...
        if v is not None:
            ...
            v_min = -1 * v_max
            v_max = 1 if v_max < 1 else v_max
            v_min = -1 if v_min > -1 else v_min

            ax.pcolormesh(np.flipud(v), cmap=cmap, vmin=v_min, vmax=v_max)
    ...
    def render_q(self, q, show_greedy_policy=True):
        ...
        q_max, q_min = max(q.values()), min(q.values())
        q_max = max(q_max, abs(q_min))
        q_min = -1 * q_max
        q_max = 1 if q_max < 1 else q_max
        q_min = -1 if q_min > -1 else q_min
            ...
            for x in range(xs):
                for action in action_space:
                    ...
                    if state == self.wall_state:
                        ...
                        tq = q[(state, action)]
                        color_scale = 0.5 + (tq / q_max) / 2  # normalize: 0.0-1.0
